...
Task ID: 9
Agent: main (Z.ai Code)
Task: User: "هیچی لود نمیشه خالیه، نوار پایین هم لود نشده" — site renders EMPTY with no bottom nav

Work Log:
- Diagnosed live: server 200, DB perfect (24 media / 6 pages), but the SSR HTML body was ONLY the loading spinner (25 chars of text) — zustand's server snapshot returns the store's INITIAL state (data:null), so the server never rendered the actual site; content appeared only after client JS executed. On slow/failed JS (mobile, cold compile) the page stayed EMPTY with no bottom nav — exactly the user's report
- Fix: MemorialApp now uses `useMemorial(s => s.data) ?? initialData` — full HTML server-rendered (site works even with JS disabled), hydration matches 1:1, no blank flash
- Secondary fix: ensureSettingRow/login read-first — no SQLite write on every page view (faster; keeps db/custom.db byte-clean for git pulls)
- Verified: raw curl HTML now 925 chars of real text + hero + both navs (was 25); /p/gallery SSR 13 imgs; /p/biography 4077 chars; /blog/[id] 200; mobile 390px preview shows full site + bottom tab bar; zero console errors; lint clean
- Committed 3ef3448 (sandbox) and pushed 5b41ea9..cbf7e86 (repo) — db/custom.db byte-identical (no diff)
