...
log_step_start "Self-heal upload API route"
# Platform boot wipes directories named "upload" — this restores our API route
if [ ! -f "$PROJECT_DIR/src/app/api/upload/route.ts" ]; then
    echo "[HEAL] src/app/api/upload/route.ts is MISSING — restoring..."
    mkdir -p "$PROJECT_DIR/src/app/api/upload"
    if [ -f "$PROJECT_DIR/.zscripts/backups/api-upload-route.ts" ]; then
        cp "$PROJECT_DIR/.zscripts/backups/api-upload-route.ts" "$PROJECT_DIR/src/app/api/upload/route.ts"
        echo "[HEAL] Restored from .zscripts/backups/api-upload-route.ts"
    else
        git -C "$PROJECT_DIR" restore --source=HEAD --worktree -- src/app/api/upload/route.ts 2>/dev/null || true
        echo "[HEAL] Restored from git HEAD (fallback)"
    fi
else
    echo "[HEAL] Upload route present — OK"
fi
log_step_end "Self-heal upload API route"
...
# Background watchdog: if anything deletes src/app/api/upload/route.ts
# mid-session, re-create it within seconds (dev server hot-reloads it back)
(
    while true; do
        if [ ! -f "$PROJECT_DIR/src/app/api/upload/route.ts" ]; then
            mkdir -p "$PROJECT_DIR/src/app/api/upload"
            ...
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] [WATCHDOG] restored upload route" >> "$PROJECT_DIR/.zscripts/watchdog.log"
        fi
        sleep 5
    done
) >/dev/null 2>&1 &
