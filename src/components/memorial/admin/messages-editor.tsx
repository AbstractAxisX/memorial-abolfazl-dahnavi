...
export function MessagesEditor({
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">{toPersianDigits(items.length)} پیام در کتاب یادبود</p>
      <div className="space-y-2">
          ...
          <div key={m.id} className={`rounded-2xl border p-4 ${m.approved ? "border-[oklch(0.74_0.135_82/0.2)] bg-ivory/50" : "border-dashed border-[oklch(0.74_0.135_82/0.25)] bg-ivory/30 opacity-70"}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {!m.approved && (
                  <span className="mt-1 inline-block rounded-full bg-[oklch(0.92_0.035_82)] px-2 py-0.5 text-[10px] text-[oklch(0.6_0.1_70)]">مخفی</span>
                )}
              </div>
              <div className="flex gap-1">
                <button
                  onClick={() => toggle(m)}
                  className={`flex h-8 w-8 items-center justify-center rounded-full transition ${m.approved ? "text-muted-foreground hover:bg-[oklch(0.95_0.018_82)]" : "text-[oklch(0.36_0.07_168)] hover:bg-[oklch(0.92_0.035_82)]"}`}
                  title={m.approved ? "مخفی کردن" : "تأیید و نمایش"}
                >
                  {m.approved ? <X className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                </button>
                <button onClick={() => remove(m.id)} className="flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:bg-[oklch(0.52_0.18_25/0.1)] hover:text-[oklch(0.52_0.18_25)]">
        ...
        {items.length === 0 && (
          <p className="rounded-2xl border border-dashed border-[oklch(0.74_0.135_82/0.25)] py-10 text-center text-sm text-muted-foreground">هنوز پیامی ثبت نشده است.</p>
        )}
      </div>
    </div>
  )
